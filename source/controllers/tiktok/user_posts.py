"""TikTok user-posts controller — fetch all posts from a user account."""
...
class TikTokUserPosts(TikTokControllers):
    """Handler that fetches all posts from a TikTok user by ``unique_id``.
    ...
    log = logging.getLogger("tiktok.user_posts")

    async def handler(self, job: dict):
        """Execute the user-posts fetch and pipe results to the output driver."""
        ...
        self.log.info(
            "Fetching user posts: unique_id=%r  count=%d  max_pages=%d  hd=%d",
        ...
        try:
            ...
            post_count = 0
            async for event in self.api.get_user_posts(
                unique_id=unique_id,
                max_pages=max_pages,
                count=cnt,
                hd=hd,
            ...
            self.log.info("User-posts complete — %d posts for %r", post_count, unique_id)
    ...
    async def scrape_to_json(self, job: dict) -> list[dict]:
        """Scrape user posts and return raw dicts — no output driver involved."""
        unique_id = self._parse_unique_id(job)
        cnt = self.parse_job_count(job)
        max_pages = self.parse_job_max_pages(job)
        hd = int(self.args.get("hd", job.get("hd", 1)))
    ...
    def _parse_unique_id(self, job: dict) -> str:
        ...
        if not unique_id:
            raise ValueError(
                "unique_id is required for --type user-posts. "
